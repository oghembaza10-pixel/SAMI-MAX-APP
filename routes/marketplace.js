// ==========================================================================
// SAMII OS — MARKETPLACE OG — Version Définitive & Dynamique (Sans Upload)
// ==========================================================================
...
const SHARED_STYLES = `
    :root {
        ...
        --bg-panel-hover: rgba(18, 18, 26, 0.95);
        --gold-og: #d4af37; 
        --gold-hover: #f3e5ab; 
        ...
        --border-cyan: 1px solid rgba(0, 240, 255, 0.4);
        --border-purple: 1px solid rgba(189, 0, 255, 0.4);
        --text-main: #f8f8f2; 
        --text-muted: #9494a0;
...
// --- 1. ACCUEIL MARKETPLACE (Grille Dynamique & Vivante) ---
router.get("/", requireAuth, async (req, res) => {
    ...
    const cardsHtml = annonces.map((a, index) => {
        const f = a.fields;
        ...
        // Effet de style dynamique alterné pour casser l'aspect plat
        const accentClass = index % 3 === 0 ? 'cyan-border' : (index % 3 === 1 ? 'gold-border' : 'purple-border');

        return `
        <a href="/vitrine/${f.vendeur_id || 'admin_og'}" class="og-card ${accentClass}">
            <div class="og-card__media">
                <div class="og-card__glow-layer"></div>
...
<head>
    ...
    <style>
        ...
        .og-publish-cta { 
            ...
            font-family: var(--font-display); box-shadow: 0 8px 30px rgba(212,175,55,0.4); 
            transition: all 0.3s var(--ease-premium); border: 1px solid #fff;
        }
        .og-publish-cta:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 12px 40px rgba(0,240,255,0.4); }
        
        /* Barre de recherche multi-couches futuriste */
        .og-amazon-search { 
        ...
        .og-amazon-search:focus-within {
            border-color: var(--cyan-tech);
            box-shadow: 0 8px 35px rgba(0,0,0,0.6), var(--cyan-glow);
        }
        ...
        /* Grille ultra-dynamique 3D-hover */
        ...
        .og-card.purple-border { border: 1px solid rgba(189, 0, 255, 0.25); }
        
        .og-card:hover { 
            transform: translateY(-8px) scale(1.02); 
            box-shadow: 0 25px 60px rgba(0,0,0,0.8), var(--cyan-glow);
        }
        ...
        .og-card__placeholder { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, rgba(0,240,255,0.05), rgba(189,0,255,0.05)); }
        ...
        .og-card__badge { 
            ...
            background: rgba(5, 5, 10, 0.85); color: #fff; border: 1px solid rgba(255,255,255,0.12); 
        ...
        .og-empty-state { grid-column: 1 / -1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 90px 20px; text-align: center; color: var(--text-muted); background: var(--bg-panel); border-radius: 24px; border: 1px dashed rgba(0,240,255,0.3); box-shadow: inset 0 0 30px rgba(0,240,255,0.05); }
    </style>
</head>
<body>
    <aside class="og-sidebar">
        ...
        <div class="og-samii-sphere">
            ...
            <div class="og-samii-text">Moteur vivant & dynamique en ligne. Architecture multi-couches active.</div>
        </div>
    </aside>
    ...
    <div class="og-main-wrapper">
        ...
        <header class="og-header">
            ...
            <form class="og-amazon-search" method="GET">
                ...
                <div class="og-search-input-box">
                    ...
                    <input type="text" name="recherche" placeholder="Rechercher un produit d'élite..." value="${recherche || ''}">
                </div>
        ...
        <main class="og-main-container">
            <div style="font-family: var(--font-mono); font-size: 0.82rem; color: var(--cyan-tech); margin-bottom: 20px; display: flex; align-items: center; gap: 8px;">
                ...
                ${annonces.length} annonce${annonces.length !== 1 ? 's' : ''} disponible${annonces.length !== 1 ? 's' : ''} sur le réseau
            </div>
            <div class="og-grid">
                ${annonces.length ? cardsHtml : `
                    <div class="og-empty-state">
                        <i data-lucide="shopping-bag" style="width:56px;height:56px;color:var(--gold-og);margin-bottom:16px;filter:drop-shadow(0 0 15px var(--gold-og));"></i>
                        <div style="font-size: 1.1rem; font-weight: 600; color: #fff; margin-bottom: 8px;">Aucune annonce active trouvée</div>
                        <div style="font-size: 0.85rem;">Publiez votre premier article ou élargissez vos critères de recherche.</div>
                    </div>
                `}
            </div>
        </main>
...
// --- 2. PAGE DE PUBLICATION (Formulaire Ultra-Stylé, Sans Upload Bloquant) ---
...
<body>
    ...
    <div class="og-form-container">
        ...
        <form action="/marketplace/publier" method="POST">
            ...
            <div class="og-form-group">
                <label>Lien Photo (URL directe optionnelle)</label>
                <input type="url" name="photo_url" class="og-form-control" placeholder="https://... (Laisser vide si pas d'image)">
            </div>
...
// --- 3. TRAITEMENT POST PUBLICATION (Sans Multer ni Erreur Cloudinary) ---
