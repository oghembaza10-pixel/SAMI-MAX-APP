...
router.get("/admin/communaute", async (req, res) => {
    try {
        ...
        const [members, posts, payments, sales, revenue, recentPosts, recentMembers] = await Promise.all([
            ...
            db.query(`SELECT COUNT(*)::int AS n FROM paiements WHERE communaute = $1 AND LOWER(COALESCE(statut,'')) IN ('paye','paid','success','succeeded','complete','completed')`, [community]),
            db.query(`SELECT COUNT(*)::int AS n FROM paiements WHERE communaute = $1 AND LOWER(COALESCE(statut,'')) IN ('paye','paid','success','succeeded','complete','completed')`, [community]),
            db.query(`SELECT COALESCE(SUM(montant),0)::numeric AS total, COALESCE(MAX(devise),'USD') AS devise FROM paiements WHERE communaute = $1 AND LOWER(COALESCE(statut,'')) IN ('paye','paid','success','succeeded','complete','completed')`, [community]),
        ...
        const pay = payments[0] || { n: 0 };
        const sale = sales[0] || { n: 0 };
        const rev = revenue[0] || { total: 0, devise: "USD" };

        res.type("html").send(`<!doctype html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Administration — ${esc(community)}</title>
<style>
:root{color-scheme:dark;--bg:#07090d;--panel:#0e1219;--line:#202734;--gold:#d9b45b;--text:#f5f7fb;--muted:#8d97a8;--green:#4ade80}
...
.wrap{max-width:1280px;margin:auto;padding:30px 22px 60px}.top{display:flex;justify-content:space-between;gap:20px;align-items:center;margin-bottom:28px}.eyebrow{color:var(--gold);font-size:12px;font-weight:800;letter-spacing:.16em;text-transform:uppercase}.title{font-size:30px;font-weight:800;margin:7px 0}.sub{color:var(--muted)}.badge{border:1px solid #5b4a25;background:#1b160b;color:#e8c86d;border-radius:999px;padding:9px 13px;font-size:13px;font-weight:700}
...
.two{display:grid;grid-template-columns:1.25fr .75fr;gap:14px;margin-top:14px}.section-title{font-size:17px;font-weight:800;margin-bottom:15px}.row{display:flex;justify-content:space-between;gap:12px;padding:13px 0;border-top:1px solid var(--line)}.row:first-of-type{border-top:0}.small{font-size:12px;color:var(--muted)}.metricline{display:flex;gap:24px;margin-top:12px;color:var(--muted);font-size:13px}.metricline b{color:var(--text)}.empty{color:var(--muted);padding:15px 0}
@media(max-width:900px){.grid{grid-template-columns:repeat(2,1fr)}.two{grid-template-columns:1fr}}@media(max-width:560px){.grid{grid-template-columns:1fr}.top{align-items:flex-start;flex-direction:column}}
</style></head><body><main class="wrap">
<header class="top"><div><div class="eyebrow">SAMII · Community Admin</div><div class="title">${esc(community)}</div><div class="sub">Bonjour ${esc(user.prenom || user.email)} — votre espace d'administration.</div></div><div class="badge">ADMIN COMMUNAUTÉ</div></header>
<section class="grid">
...
<div class="card"><div class="label">Ventes</div><div class="num accent">${sale.n}</div><div class="small">Transactions payées</div></div>
</section>
<section class="grid" style="margin-top:14px">
<div class="card"><div class="label">Chiffre d'affaires</div><div class="num accent">${Number(rev.total || 0).toLocaleString("fr-FR",{maximumFractionDigits:2})} ${esc(rev.devise || "USD")}</div></div>
<div class="card"><div class="label">Engagement total</div><div class="num">${Number(p.likes||0)+Number(p.comments||0)+Number(p.shares||0)}</div></div>
</section>
<section class="two">
<div class="card"><div class="section-title">Dernières publications</div>${recentPosts.length ? recentPosts.map(x=>`<div class="row"><div><div>${esc((x.contenu||"").slice(0,110))}${(x.contenu||"").length>110?"…":""}</div><div class="small">${new Date(x.created_at).toLocaleString("fr-FR")}</div></div><div class="small">♥ ${x.like_count||0} · 💬 ${x.commentaire_count||0} · ↗ ${x.partage_count||0}</div></div>`).join("") : '<div class="empty">Aucune publication pour le moment.</div>'}</div>
